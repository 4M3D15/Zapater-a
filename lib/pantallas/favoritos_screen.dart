...
import '../widgets/particle_explosion.dart'; // Asegúrate de crear este archivo
...
class _FavoritosScreenState extends State<FavoritosScreen> {
  ...
  final List<Widget> _explosiones = [];
  ...
  void _eliminarConExplosion(Producto p, Offset position) async {
    setState(() {
      _explosiones.add(
        ParticleExplosion(
          position: position,
              ...
              Row(
                ...
                children: [
                  IconButton(
                    ...
                    onPressed: () => context.read<FavoritosModel>().vaciarFavoritos(),
                  ),
                ],
              ),
              Expanded(
                                    ...
                                    children: [
                                      ...
                                      Positioned(
                                        ...
                                        child: AnimatedFavoriteIcon(
                                          esFavorito:
                                          context.watch<FavoritosModel>().esFavorito(p),
                                          onTap: () {
                                            final favs = context.read<FavoritosModel>();
                                            favs.esFavorito(p)
                                                ? favs.removerFavorito(p)
  ...
  Widget _buildFavoriteTile(Producto p) {
    return GestureDetector(
      onTapDown: (details) {
        _eliminarConExplosion(p, details.globalPosition);
      },
      child: Card(
        ...
        child: ListTile(
          ...
          leading: Image.network(p.imagen, width: 50, height: 50, fit: BoxFit.cover),
          title: Text(p.nombre),
